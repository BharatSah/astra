    ...
    <div className="animate-slide-up flex flex-col" style={{ minHeight: 'calc(100vh - 7rem)' }}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4 animate-slide-up">
          <div className="w-full max-w-md glass-panel border border-slate-800 rounded-2xl shadow-2xl shadow-brand-500/10 flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {editingId ? 'Update saved login details.' : 'Securely save a new platform credential.'}
                  </p>
                </div>
              </div>
              <button
              ...
              {platforms.length === 0 ? (
                <div className="py-12 text-center text-sm text-slate-400 space-y-4">
                  ...
                  <button
                    onClick={() => {
                      setIsModalOpen(false);
                      onNotify('warning', 'Navigate to Settings > Platform Registry to configure platforms');
                    }}
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-[10px] font-bold text-brand-400 hover:text-brand-300 transition-colors cursor-pointer"
                      >
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-4 pr-12 py-3 rounded-xl text-slate-200 glass-input text-sm font-mono tracking-wider transition-all duration-200 focus:scale-[1.01]"
                        required
                      />
                      {password && (
                        <button
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-slate-400 hover:text-brand-400 hover:bg-slate-800/60 transition-all duration-200 cursor-pointer"
                          title="Copy password"
                        >
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      {[1, 2, 3, 4].map((bar) => {
                        ...
                        const color = strength >= bar ? (strength === 4 ? 'bg-emerald-500' : strength === 3 ? 'bg-amber-400' : 'bg-rose-500') : 'bg-slate-800';
                        return (
                          <span
                            key={bar}
